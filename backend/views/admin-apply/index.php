            ...
            <div class="box">
                ...
                <div class="box-body">
                    <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <?php ActiveForm::begin([ 'method'=>'get','action'=>Url::toRoute(['admin-apply/index']), 'options' => ['class' => 'form-inline']]); ?>
                                <div class="form-group" style="margin: 5px;">
                                    <input type="text" class="form-control" name="query[name]"  placeholder="用户姓名/手机号" value="<?= isset($query['name']) ? $query['name'] : ''?>">
                                </div>
                                <div class="form-group" style="margin: 5px;">
                                    <input type="text" class="form-control" name="query[product_name]"  placeholder="产品" value="<?= isset($query['product_name']) ? $query['product_name'] : ''?>">
                                </div>
                                <div class="form-group" style="margin: 5px;">
                        ...
                        <div class="row">
                            <div class="box-tools" style="padding: 10px 0">
                                <div class="input-group input-group-sm" style="padding-left: 20px">
                                    ...
                                    <a href="<?= Url::toRoute(['admin-apply/export','query'=>$query])?>"><button id="export_btn" type="button" class="btn btn-xs btn-primary">导&nbsp;&emsp;出</button></a>
                                </div>
                            </div>
                        </div>
                        <!-- row start -->
                                ...
                                <table id="data_table" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="data_table_info">
                                    ...
                                    <?php foreach($models as $model):?>
                                        <tr>
                                            ...
                                            <td><?= date('Y-m-d H:i:s',$model['b_time'])?></td>
                                            <td><?= date('Y-m-d H:i:s',$model['e_time'])?></td>
                                            <td><span class="label <?php $color=['1'=>'bg-gray','2'=>'bg-blue','3'=>'bg-red','4'=>'bg-purple','5'=>'bg-green'];echo $color[$model['status']]?>"><?php $status=['1'=>'待审核','2'=>'审批通过','3'=>'审批未通过','4'=>'已放款','5'=>'已还款']; echo $status[$model['status']] ?></span></td>
                                            <td><?= $model['other_info_val']?></td>
                                            <td>
...
<div id="audit_content" class="form-group form-inline" style="display: none;margin-top: 30px;margin-left: 50px">
    <div class="input-group">
        ...
        <select class="form-control" name="status" style="width: 150px">
            ...
            <option class="status" value="2">审批通过</option>
            <option class="status" value="3">审批未通过</option>
            <option class="status" value="4">已放款</option>
            <option class="status" value="5">已还款</option>
        </select>
    </div>
</div>
...
<script>
    ...
    function deleteAction(id) {
        layer.confirm('确定要删除吗？',{},function () {
            $.get('<?= Url::toRoute(['admin-apply/delete'])?>',{id:id},function (res) {
                if(res.status === 200){
    ...
    $('#delete_btn').click(function() {
        ...
        if ($('tbody input:checked').length === 0) {
            ...
            layer.confirm('确定要删除吗？', {}, function () {
                ...
                $.post('<?=Url::toRoute(['admin-apply/batch-del'])?>', {ids: ids, _csrf: csrfToken}, function (res) {
                    if (res.status === 200) {
                        layer.msg(res.msg, {icon: 1, time: 1500}, function () {
                            location.reload();
                        })
        ...
        layer.open({
           ...
           content:$('#audit_content'),
            yes:function () {
                ...
                $.get('<?= Url::toRoute(['admin-apply/audit'])?>',{id:id,status:real_status},function (res) {
                        if(res.status === 200){
                            layer.msg(res.msg,{icon:1,time:1500},function () {
                                location.reload();
                            });
