        ...
        <div class="change-pwd-box">
            <form action="" method="post" onsubmit="return false;">
                <div class="item">
                    ...
                    <input type="text" name="phone" placeholder="请输入手机号" value="<?= $member['tel']?>" disabled>
                </div>
                <div class="item">
...
<script>
    ...
    $('.btn button').click(function () {
        ...
        if(check(phone,captcha,password,re_password)){
            $.post('<?=  Url::toRoute(['member/change-pwd'])?>',{_csrf:_csrf,phone:phone,captcha:captcha,password:password,re_password:re_password},function (res) {
                console.log(res);
                if(res.status === 200){
